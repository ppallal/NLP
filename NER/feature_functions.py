...
class FeatureFunctions(object):
  ...
  #   def fPrice_7(self, h, tag): # checking for Other Price
    ...
    def fPrice_3(self, h, tag): # if its a number
		if(type(h[0])==type((2,3))):
        	   h = h[0]
    ...
    def fPrice_4(self, h, tag): # checking its a number and the previous one is Price
		if(type(h[0])==type((2,3))):
        	   h = h[0]
    ...
    def fPrice_5(self, h, tag): # if its currency
		if(type(h[0])==type((2,3))):
	           h = h[0]
		if tag != "Price":
			return 0
    ...
    def fPrice_6(self, h, tag): # if current 
		if(type(h[0])==type((2,3))):
	           h = h[0]
		if tag != "Price":
			return 0
