...
class MyMaxEnt(object):
    def __init__(self, datast, function_obj, reg_lambda = 0.01, pic_file = None): 
        ...
        self.tag_set = ["price_query","feature_query","comparison","interest_intent","acknowledgement","irrelevant","offensive","greeting","agreement","disagreement"]#self.func.supported_tags #None # this will be also be set by create_dataset - this is the set of all tags
        self.create_dataset()
    ...
    def create_dataset(self):
        self.dataset = []
        self.all_data = {}
        rand_no = randint(0,(len(h_tuples)-7502))
        for h in self.datast[rand_no:-1]: # h represents each element of datast ie. [sentence,(tags),relation]   [:9000]
            for relation in self.tag_set:
                feats = self.all_data.get(relation, [])
                val = self.get_feats(h, relation)
                feats.append(val)
                self.all_data[relation] = feats
                if (h[2] == relation):    # hand-tag relation
...
datast = build_datast(data);
fun_obj = feature_functions(tag_list)
maxent = MyMaxEnt(datast,fun_obj,reg_lambda = 0.01, pic_file = None)
maxent.train()
result=[]
for i in datast[4000:4050]:
    result.append(maxent.classify(i))
print(result)
